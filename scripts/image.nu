...
def "main build image" [
    ...
    if $push {
        ...
        TAG=$tag IMAGE=$"($registry)/($image)" PLATFORM=$platform docker buildx bake $bake_target --push

    } else {

        TAG=$tag IMAGE=$"($registry)/($image)" PLATFORM=$platform docker buildx bake $bake_target
